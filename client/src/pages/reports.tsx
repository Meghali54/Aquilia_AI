...
import { generatePDFReport, generateExcelReport, generateCSVReport } from '@/lib/reportGenerator';
import { 
  FileText, 
  Download, 
  BarChart3, 
...
export default function Reports() {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const progressSteps = [
        ...
        { step: 'Finalizing and downloading...', progress: 100 },
      ];
      ...
      const reportData = {
        ...
        datasets: reportConfig.datasets.length > 0 ? reportConfig.datasets : ['Sample Dataset'],
      ...
      const newReport = {
        ...
        size: reportConfig.format === 'pdf' ? '2.1 MB' : reportConfig.format === 'excel' ? '1.8 MB' : '0.5 MB',
      };
  ...
  const handleDownloadReport = async (reportId: string) => {
    ...
    try {
      // Prepare report data for re-generation
      const reportData = {
        title: report.title,
        type: report.type,
                            ...
                            <Button
                              onClick={async (e) => {
                                ...
                                const quickReportData = {
                                  title: `${type.name} - ${new Date().toLocaleDateString()}`,
                                  type: type.name,
                                  generatedAt: new Date(),
                                ...
                                try {
                                  toast({
                                    ...
                                    description: "Creating PDF with sample data",
                                  });
                                  
                                  await generatePDFReport(quickReportData);
                                  
                                  toast({
                                    title: "Sample report downloaded!",
                                    description: `${type.name} PDF has been generated and downloaded.`,
                                  });
                                } catch (error) {
                                  toast({
                              ...
                              variant="outline"
                              size="sm"
                              className="w-full border-white/20 text-white hover:bg-white/10 hover:border-blue-400/50 rounded-xl"
                            >
