...
import { Database, Gauge, Dna, Brain, Upload, FileText, Zap, TrendingUp, Activity, Waves, AlertTriangle } from 'lucide-react';
...
// Ocean monitoring data for interactive charts
const oceanData = [
  { month: 'Jan', temperature: 14.2, salinity: 34.8, ph: 8.1, dissolved_oxygen: 7.2 },
  { month: 'Feb', temperature: 13.8, salinity: 34.9, ph: 8.0, dissolved_oxygen: 7.4 },
  { month: 'Mar', temperature: 15.1, salinity: 34.7, ph: 8.2, dissolved_oxygen: 7.1 },
  { month: 'Apr', temperature: 17.3, salinity: 34.6, ph: 8.1, dissolved_oxygen: 6.9 },
  { month: 'May', temperature: 19.8, salinity: 34.5, ph: 8.0, dissolved_oxygen: 6.7 },
  { month: 'Jun', temperature: 22.4, salinity: 34.4, ph: 7.9, dissolved_oxygen: 6.5 },
  { month: 'Jul', temperature: 24.1, salinity: 34.3, ph: 7.8, dissolved_oxygen: 6.3 },
  { month: 'Aug', temperature: 25.2, salinity: 34.2, ph: 7.7, dissolved_oxygen: 6.1 },
  { month: 'Sep', temperature: 23.7, salinity: 34.4, ph: 7.8, dissolved_oxygen: 6.4 },
  { month: 'Oct', temperature: 21.1, salinity: 34.6, ph: 8.0, dissolved_oxygen: 6.8 },
  { month: 'Nov', temperature: 18.4, salinity: 34.7, ph: 8.1, dissolved_oxygen: 7.0 },
  { month: 'Dec', temperature: 15.9, salinity: 34.8, ph: 8.1, dissolved_oxygen: 7.3 }
];

const speciesData = [
  { name: 'Cod', count: 342, color: '#3B82F6' },
  { name: 'Herring', count: 289, color: '#10B981' },
  { name: 'Mackerel', count: 156, color: '#F59E0B' },
  { name: 'Sardine', count: 98, color: '#EF4444' },
  { name: 'Anchovy', count: 67, color: '#8B5CF6' }
];

const alertData = [
  { parameter: 'Temperature', value: 25.2, threshold: 24.0, status: 'warning', unit: '°C' },
  { parameter: 'pH', value: 7.7, threshold: 7.8, status: 'critical', unit: '' },
  { parameter: 'Dissolved O₂', value: 6.1, threshold: 6.5, status: 'warning', unit: 'mg/L' },
  { parameter: 'Salinity', value: 34.2, threshold: 35.0, status: 'normal', unit: 'PSU' }
];
      ...
      <div className="bg-slate-800/95 backdrop-blur-sm border border-slate-600/50 rounded-lg p-3 shadow-xl">
        ...
        {payload.map((pld: any, index: number) => (
          <div key={index} className="flex items-center gap-2 text-sm">
            ...
            ></div>
            <span className="text-slate-300">{pld.dataKey}:</span>
            <span className="text-white font-medium">
              {pld.value}
              {pld.dataKey === 'temperature' ? '°C' : 
               pld.dataKey === 'salinity' ? ' PSU' : 
               pld.dataKey === 'dissolved_oxygen' ? ' mg/L' : ''}
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <Layout breadcrumbs={breadcrumbs}>
      <div className="p-6 space-y-8 relative">
        ...
        {/* Interactive Ocean Monitoring Charts */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
          {/* Ocean Temperature & Salinity Trends */}
          <Card className="bg-gradient-to-br from-slate-800/80 to-slate-700/80 backdrop-blur-sm border-slate-700/50 hover:border-slate-600/50 transition-all duration-300">
            <CardHeader className="border-b border-slate-700/50">
          ...
          <Card className="bg-gradient-to-br from-slate-800/80 to-slate-700/80 backdrop-blur-sm border-slate-700/50 hover:border-slate-600/50 transition-all duration-300">
            <CardHeader className="border-b border-slate-700/50">
              <CardTitle className="text-xl font-bold bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent flex items-center gap-2">
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-slate-800/95 backdrop-blur-sm border border-slate-600/50 rounded-lg p-3 shadow-xl">
                              ...
                              <p className="text-slate-300 text-sm">Count: <span className="text-white font-medium">{data.value}</span></p>
                            </div>
                          );
                        }
                        return null;
              ...
              <div className="grid grid-cols-2 gap-2 mt-4">
                {speciesData.map((species, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-white font-medium ml-auto">{species.count}</span>
                  </div>
                ))}
              </div>
            </CardContent>
                ...
                {alertData.map((alert, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-slate-300 font-medium">{alert.parameter}</span>
                    ...
                    <div className="space-y-1">
                      <p className={`text-lg font-bold ${
                        ...
                        alert.status === 'warning' ? 'text-yellow-400' : 'text-green-400'
                      }`}>
                      ...
                      <Badge 
                        className={`text-xs ${
                          ...
                          alert.status === 'warning' ? 'bg-yellow-500/20 text-yellow-400' : 
